...
class AnimalShogiNNetA(nn.Module):
    filename_prefix = 'a_'
    ...
    def __init__(self, game, args):
        ...
        super(AnimalShogiNNetA, self).__init__()
...
class AnimalShogiNNetB(nn.Module):
    filename_prefix = 'b_'
    ...
    def __init__(self, game, args):
        ...
        super(AnimalShogiNNetB, self).__init__()
        
        self.board_layers = nn.Sequential(
            nn.Conv2d(10, args.num_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(args.num_channels),
            nn.ReLU(),
            nn.Conv2d(args.num_channels, args.num_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(args.num_channels),
    ...
    def forward(self, boards, motis):
        ...
        boards = boards.view(-1, 1, 3, 4)
        split_boards = torch.zeros(batch_size, 11, 3, 4).contiguous().cuda()
                                                                     # batch_size x 1 x 3 x 4
        for i in range(batch_size):
            for j in range(3):
                for k in range(4):
                    split_boards[i, int(boards[i, 0, j, k]) - 1, j, k] = 1

        motis = motis.view(-1, 1, 2, 3)                              # batch_size x 1 x 6
        
        boards = self.board_layers(split_boards[:, 1:, :, :])        # batch_size x num_channels x 3 x 4
        motis = self.moti_layers(motis)                              # batch_size x num_channels x 3 x 4
